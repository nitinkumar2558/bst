...
// int rangeSearch(int k1, int k2) ->range search: given two values k1 and k2, print all the elements (or keys) x in the BST such that k1 <= x <= k2. Also count the number of elements in the range from k1 to k2 and returns it. 
 ...
 int heighthelper(){
   ...
   //base case
           if(current==NULL) return 0;//crucial ,at end this give left=0 then it 
           //give right =0 that actually initialising left and right
           //each time left and right of each node fuction returns left+right+1 for count
           //here if left is more then add node itself and return this back above vice versa
        else 
        {
